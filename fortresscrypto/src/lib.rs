...
mod tests {
	...
	use rand::{rand_core::UnwrapErr, rngs::SysRng, seq::IndexedMutRandom, RngExt};
	use std::io::Cursor;
	...
	fn test_network_key_suite() {
		...
		// Encrypt and decrypt
		let plaintext = (0..2017).map(|_| UnwrapErr(SysRng).random()).collect::<Vec<u8>>();
		let id: [u8; 32] = UnwrapErr(SysRng).random();
		let bad_id: [u8; 32] = UnwrapErr(SysRng).random();
	...
	fn file_corruption() {
		...
		for _ in 0..64 {
			let mutation_byte: u8 = UnwrapErr(SysRng).random();

			let truncated = &encrypted_data[..encrypted_data.len() - UnwrapErr(SysRng).random_range(1..encrypted_data.len())];
			let corrupted_checksum = {
				let mut buffer = encrypted_data.clone();
				buffer.choose_mut(&mut UnwrapErr(SysRng)).map(|x| *x ^= mutation_byte);
				buffer
			};
			let corrupted_mac = {
				...
				data[32..].choose_mut(&mut UnwrapErr(SysRng)).map(|x| *x ^= mutation_byte);
