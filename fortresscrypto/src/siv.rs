...
mod tests {
	...
	fn test_deterministic_encryption() {
		...
		let data = (0..1034).map(|_| UnwrapErr(SysRng).random()).collect::<Vec<u8>>();
		let keys = SivEncryptionKeys {
			siv_key: UnwrapErr(SysRng).random(),
			cipher_key: UnwrapErr(SysRng).random(),
		};
	...
	fn test_different_keys() {
		let id1: [u8; 32] = UnwrapErr(SysRng).random();
		let id2: [u8; 32] = UnwrapErr(SysRng).random();
		let data = (0..1034).map(|_| UnwrapErr(SysRng).random()).collect::<Vec<u8>>();
		let keys = SivEncryptionKeys {
			siv_key: UnwrapErr(SysRng).random(),
			cipher_key: UnwrapErr(SysRng).random(),
